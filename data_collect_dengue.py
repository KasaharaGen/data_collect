...
# ログ設定
# -------------------------
logging.basicConfig(filename="data_collect_dengue.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")

# -------------------------
# Europe PMC クエリ（PUB_YEAR を除外）
# -------------------------
QUERY_EPMC = '("dengue*" OR "dengue virus" OR "dengue fever")'

# -------------------------
# PubMed クエリ
# -------------------------
BASE_QUERY_PUBMED = f'(""dengue*" OR "dengue virus" OR "dengue fever") AND (2015[PDAT] : {CURRENT_YEAR}[PDAT])'
...
def main():
    ...
    df_epmc.to_csv("dengue_epmc.csv", index=False)
    ...
    df_pubmed.to_csv("dengue_pubmed.csv", index=False)
    ...
    merged.to_csv("dengue_merged.csv", index=False)

    print("\n✅ 保存完了:")
    print(" - dengue_epmc.csv")
    print(" - dengue_pubmed.csv")
    print(" - dengue_merged.csv")
